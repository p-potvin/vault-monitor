...
export default function HeatmapGrid({ daySeries, t }: HeatmapGridProps) {
    ...
    if (!weeks.length) return <p className="text-[12px] italic text-vault-muted">{t.noAgentDays}</p>;
                    ...
                    <div key={wi} className="flex flex-col gap-[3px] shrink-0">
                        {week.map(({ date, count }) => (
                            <Tooltip key={date} content={`${date}: ${count} event${count !== 1 ? "s" : ""}`}>
                                <div className={`w-[13px] h-[13px] rounded-[2px] cursor-default transition-opacity hover:opacity-80 ${levelClass(count)}`} aria-label={`${date}: ${count}`} />
                            </Tooltip>
                        ))}
                    </div>
                ))}
