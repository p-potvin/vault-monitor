...
export function ResourcesPage({ setLoading }: { setLoading: (loading: boolean) => void }) {
  ...
  return <main className="view-stack resource-page"><header className="resource-page-head"><p>{t.subtitle}</p><span className="resource-refresh">{t.refresh}</span></header>{error && <div className="error-line">{error}</div>}<div className="resource-fleet">{hosts.map((host) => <HostCard host={host} t={t} expanded={expanded === host.id} onToggle={() => setExpanded((current) => current === host.id ? null : host.id)} key={host.id} />)}</div></main>;
}
