...
This repo owns UI, routing, view state, and display components only. Normalized data comes from `vaultwares-api`:
...
The Vite dev server proxies `/monitor/*` to `https://api.vaultwares.ca` by default. Override that with `VITE_MONITOR_API_TARGET`.

## Data Boundary

The browser must not read `agent-ledger`, `health-ledger`, or Kiwi data directly. `vaultwares-api` owns adapters and normalization.
